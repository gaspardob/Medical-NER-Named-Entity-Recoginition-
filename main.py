# exemple donné de rapport sous ce format
input_json = """ 
{
  "pages": [
    {
...
# TROUVONS LE PRENOM ET LE NOM DU PATIENT
...
for i in range(nombre_mots):
    if (
        ...
        if text[i + 1].isupper():  # si c'est une majuscule, on a juste le NOM
            ...
            prenom.append(text[i + 1])  # prenom
            nom.append(text[i + 2])  # NOM
    if text[i] == "Nom:":
        nom.append(text[i + 1])
    if text[i] == "Prénom":
...
if (
...
):  # si on n'a pas le prenom et le nom, on va au moins récuperer le nom par une autre méthode (si il est écris en majuscule)
    for i in range(nombre_mots):
        prenoms_docteurs = []
        noms_docteur = []
    ...
    for j in range(nombre_mots):
        if text[j].isupper() and text[j] not in noms_docteur:
            nom.append(text[j])

print("Prénom:", prenom[0], "Nom:", nom[0])
